...
```{stan, output.var = "bt_model_stan", eval = FALSE}
data {
...
```{r stan_model_fit, results = "hide", eval = FALSE}
...
m_stan <- rstan::sampling(bt_model_stan, data = stan_data, warmup = 2000, iter = 8000)

# store fitted Stan model
save(m_stan, file="bt_model_stan_fit.Rds")
...
summ <- broom.mixed::tidy(m_stan, pars = "team_OR", conf.int = TRUE)
...
ggplot(summ[order(summ$estimate),],
       aes(y = estimate,x = factor(term, level = term), color = color)) +
  geom_point(size = 3) +
